...
from myutils.get_influx_data import query_field_val_from_db, get_bucket



def last_indoors_temperature():
    temperature_indoors_query = "from(bucket: \"{}\")\
    |> range(start: -1h)\
    |> filter(fn: (r) => r._measurement == \"sensor_temperature\")\
    |> filter(fn: (r) => r._field == \"temperature\")\
    |> filter(fn: (r) => r.host == \"rpizero2\")\
    |> last()".format(get_bucket())
...
def last_outdoors_temperature():
    temperature_outdoors_query= "from(bucket: \"{}\")\
    |> range(start: -1h)\
    |> filter(fn: (r) => r._measurement == \"sensor_temperature\")\
    |> filter(fn: (r) => r._field == \"temperature\")\
...
def last_indoors_pressure():
    pressure_indoors_query = "from(bucket: \"{}\")\
        |> range(start: -1h)\
...
def last_outdoors_pressure():
    pressure_outdoors_querry = "from(bucket: \"{}\")\
        |> range(start: -1h)\
        |> filter(fn: (r) => r._measurement == \"sensor_pressure\")\
        |> filter(fn: (r) => r._field == \"pressure\")\
        |> filter(fn: (r) => r.host == \"telegraf-docker\")\
...
def last_indoors_humidity():
    humidity_indoors_querry = "from(bucket: \"{}\")\
        |> range(start: -1h)\
...
def last_outdoors_humidity():
    humidity_outdoors_querry = "from(bucket: \"{}\")\
        |> range(start: -1h)\
        |> filter(fn: (r) => r._measurement == \"sensor_humidity\")\
        |> filter(fn: (r) => r._field == \"humidity\")\
        |> filter(fn: (r) => r.host == \"telegraf-docker\")\
...
def last_indoors_light():
    light_indoors_query= "from(bucket: \"{}\")\
        |> range(start: -1h)\
